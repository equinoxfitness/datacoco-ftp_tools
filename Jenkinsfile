...
pipeline{
    ...
    stages{
        stage('Coverage to Codacy'){
            steps {
                slackSend (color: 'good', message: "datacoco-ftp_tools_pypi_pipeline_${GIT_BRANCH} - Starting build #${BUILD_NUMBER}. (<${env.BUILD_URL}|Open>)")
...
//                 sh "python-codacy-coverage -r coverage.xml"
    ...
    post {
        failure {
            echo "fail"
            slackSend (color: 'danger', message: "@here datacoco-ftp_tools_pypi_pipeline_${GIT_BRANCH} - Build #${BUILD_NUMBER} Failed. (<${env.BUILD_URL}|Open>)")
        }
        success {
            echo "good"
            slackSend (color: 'good', message: "datacoco-ftp_tools_pypi_pipeline_${GIT_BRANCH} - Build #${BUILD_NUMBER} Success. (<${env.BUILD_URL}|Open>)")
        }
        always {
